...
namespace Microsoft.Dynamics365.UIAutomation.Api.UCI
{
    ...
    public class Field
    {
        ...
        /// <summary>
        /// Gets or sets the Readonly attribute of the field.
        /// </summary>
        /// <value>The Readonly attribute</value>
        public string ReadonlyAttribute { get; set; }
        ...
        public bool IsReadOnly
        {
            get
            {
                if (containerElement.HasElement(By.XPath(AppElements.Xpath[AppReference.Field.ReadOnly])))
                {
                    ...
                    if (readOnly.HasAttribute(ReadonlyAttribute ?? "aria-readonly"))
                    {
                    ...
                    else if (readOnly.HasAttribute(ReadonlyAttribute ?? "readonly"))
                        return true;
                }
                else if (containerElement.HasElement(By.TagName("select")))
                {
                    ...
                    if (readOnlySelect.HasAttribute(ReadonlyAttribute ?? "disabled"))
                        return true;
                ...
                else if (containerElement.HasElement(By.TagName("input")))
                {
                    ...
                    if (readOnlyInput.HasAttribute(ReadonlyAttribute ?? "disabled"))
                        return true;
                }
                else
                {
        ...
        public string ToolTip
        {
            get
            {
                if (containerElement.HasElement(By.XPath(AppElements.Xpath[AppReference.Field.ToolTip])))
                {
                    var required = containerElement.FindElement(By.XPath(AppElements.Xpath[AppReference.Field.ToolTip]));

                    if (required != null)
                        return required.GetAttribute("title");
                }
