...
namespace Microsoft.Dynamics365.UIAutomation.Api.UCI
{
    public class WebClient : BrowserPage
    {
        ...
        {
            var input = driver.WaitUntilAvailable(By.XPath(Elements.Xpath[Reference.Login.LoginPassword]), new TimeSpan(0, 0, 3));
            Thread.Sleep(1000);
            if (input == null)
            {
                var errorPage = driver.WaitUntilAvailable(By.XPath(Elements.Xpath[Reference.Login.ErrorPage]), new TimeSpan(0, 0, 2));
                if (errorPage != null)
                {
                    throw new Exception("Error page displayed during login");
                }
                input = driver.WaitUntilAvailable(By.XPath(Elements.Xpath[Reference.Login.LoginPassword]));
                Thread.Sleep(1000);
            }
        ...
        internal BrowserCommandResult<bool> OpenApp(string appName, int thinkTime = Constants.DefaultThinkTime, bool throwOnNotFound = true)
        {
            ThinkTime(thinkTime);
        ...
        internal BrowserCommandResult<bool> CloseModalDialog()
        {
            return this.Execute(GetOptions($"Close Modal Dialog"), driver =>
            {
                ...
                if (inlineDialog)
                {
                    var dialogHeader = driver.WaitUntilAvailable(By.XPath(Elements.Xpath[Reference.Dialogs.ModalHeader]));

                    if (
                        !(dialogHeader?.FindElements(By.XPath(Elements.Xpath[Reference.Dialogs.ModalOkButton])).Count >
                          0)) return true;
                    var okBtn = dialogHeader.FindElement(By.XPath(Elements.Xpath[Reference.Dialogs.ModalOkButton]));
                    okBtn.Click();
                }
        ...
        internal BrowserCommandResult<bool> CloseErrorDialog()
        {
            return this.Execute(GetOptions($"Close Error Dialog"), driver =>
            {
                ...
                if (inlineDialog)
                {
                    ...
                    if (
                        !(dialogHeader?.FindElements(By.XPath(AppElements.Xpath[AppReference.Dialogs.CancelButton])).Count >
                          0)) return true;
                    var okBtn = dialogHeader.FindElement(By.XPath(AppElements.Xpath[AppReference.Dialogs.CancelButton]));
                    okBtn.Click();
                }
        ...
        internal BrowserCommandResult<bool> RejectionDialog(bool ClickOkButton, string rejectionReason = null, string otherReason = null)
        {
            //Passing true clicks the confirm button.  Passing false clicks the Cancel button.
            return this.Execute(GetOptions($"Ok or Cancel Rejection Dialog"), driver =>
            {
                var inlineDialog = Browser.Driver.WaitUntilAvailable(By.XPath(Elements.Xpath[Reference.Frames.FullPageWebResourceFrame]),
                    TimeSpan.FromSeconds(15),
                    d => { Browser.Driver.SwitchTo().Frame(Elements.ElementId[Reference.Frames.FullPageWebResourceFrameId]); });

                 if (inlineDialog != null)
                 {
                //Wait until the buttons are available to click
                var dialogFooter = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.RejectOkButton]));

                    if (
                        !(dialogFooter?.FindElements(By.XPath(AppElements.Xpath[AppReference.Dialogs.RejectOkButton])).Count >
                          0)) return true;
                    ...
                    {
                        var inputReason = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.RejectReason]));
                        inputReason.Click();
                        var inputReasonItem = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.RejectReasonItem].Replace("[NAME]", rejectionReason)));
                        Thread.Sleep(500);
                        inputReasonItem.Click();                        
                    }
                    if (!string.IsNullOrEmpty(otherReason))
                    {
                        var inputOtherReason = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.RejectOtherReason]));
                        Thread.Sleep(500);
                        inputOtherReason.SendKeys(otherReason);
                    }

                    //Click the Confirm or Cancel button
                    IWebElement buttonToClick;
                    if (ClickOkButton)
                        buttonToClick = dialogFooter.FindElement(By.XPath(AppElements.Xpath[AppReference.Dialogs.RejectOkButton]));
                    else
                        buttonToClick = dialogFooter.FindElement(By.XPath(AppElements.Xpath[AppReference.Dialogs.RejectCancelButton]));
        ...
        internal BrowserCommandResult<bool> ReassignDialog(bool ClickOkButton, string userOrTeam, string reassignmentReason = null)
        {
            //Passing true clicks the confirm button.  Passing false clicks the Cancel button.
            return this.Execute(GetOptions($"Reassign Dialog"), driver =>
            {
                var inlineDialog = Browser.Driver.WaitUntilAvailable(By.XPath(Elements.Xpath[Reference.Frames.FullPageWebResourceFrame]),
                    TimeSpan.FromSeconds(15),
                    d => { Browser.Driver.SwitchTo().Frame(Elements.ElementId[Reference.Frames.FullPageWebResourceFrameId]); });
                ...
                {
                    //Wait until the buttons are available to click
                    var dialogFooter = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.ReassignCancelButton]));

                    if (
                        !(dialogFooter?.FindElements(By.XPath(AppElements.Xpath[AppReference.Dialogs.ReassignCancelButton])).Count >
                          0)) return true;

                    var userOrTeamInput = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.ReassignUserOrTeam]));
                    userOrTeamInput.Click();

                    Thread.Sleep(1000);

                    Browser.Driver.SwitchTo().ParentFrame();
                    //enter user or team
                    SimpleLookup(userOrTeam);

                    Browser.Driver.SwitchTo().Frame(Elements.ElementId[Reference.Frames.FullPageWebResourceFrameId]);
                    if (!string.IsNullOrEmpty(reassignmentReason))
                    {
                        var inputReason = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.ReassignReason]));
                        inputReason.Click();
                        var inputReasonItem = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.ReassignReasonItem].Replace("[NAME]", reassignmentReason)));
                        Thread.Sleep(500);
                        inputReasonItem.Click();
                    }
                    ...
                    IWebElement buttonToClick;
                    if (ClickOkButton)
                        buttonToClick = dialogFooter.FindElement(By.XPath(AppElements.Xpath[AppReference.Dialogs.ReassignOkButton]));
                    else
                        buttonToClick = dialogFooter.FindElement(By.XPath(AppElements.Xpath[AppReference.Dialogs.ReassignCancelButton]));
        ...
        internal BrowserCommandResult<bool> ReportDialog(bool ClickRunReport)
        {
            //Passing true clicks the confirm button.  Passing false clicks the Cancel button.
            return this.Execute(GetOptions($"Ok or Cancel Rport Dialog"), driver =>
            {
                var inlineDialog = Browser.Driver.WaitUntilAvailable(By.XPath(Elements.Xpath[Reference.Frames.InlineDialogIFrame]),
                    TimeSpan.FromSeconds(15),
                    d => { Browser.Driver.SwitchTo().Frame(Elements.ElementId[Reference.Frames.InlineDialogIFrameId]); });

                if (inlineDialog != null)
                {
                    //Wait until the buttons are available to click
                    var dialogFooter = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.ReportOkButton]));

                    if (
                        !(dialogFooter?.FindElements(By.XPath(AppElements.Xpath[AppReference.Dialogs.ReportOkButton])).Count >
                          0)) return true;
                    ...
                    IWebElement buttonToClick;
                    if (ClickRunReport)
                        buttonToClick = dialogFooter.FindElement(By.XPath(AppElements.Xpath[AppReference.Dialogs.ReportOkButton]));
                    else
                        buttonToClick = dialogFooter.FindElement(By.XPath(AppElements.Xpath[AppReference.Dialogs.ReportCancelButton]));
        ...
        internal BrowserCommandResult<bool> DuplicateDetection(bool clickSaveOrCancel)
        {
            ...
            return this.Execute(GetOptions($"{operationType} Duplicate Detection Dialog"), driver =>
            {
                ...
                {
                    //Wait until the buttons are available to click
                    var dialogFooter = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.DuplicateDetectionIgnoreSaveButton]), TimeSpan.FromSeconds(5));
        ...
        internal BrowserCommandResult<bool> SimpleLookup(string lookupValue)
        {
            return this.Execute(GetOptions($"Simple Lookup Dialog"), driver =>
            {
                var input = driver.ClickWhenAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.SimpleLookupInput]), "Simple lookup input field unavailable");
                input.SendKeys(lookupValue, true);

                ThinkTime(2000);

                //Pick the result from the list
                var resultsContainer = driver.WaitUntilVisible(By.XPath(AppElements.Xpath[AppReference.Dialogs.SimpleLookupListItem].Replace("[NAME]",lookupValue)));
                if (resultsContainer == null)
                {
                    throw new InvalidOperationException("Could not find lookup value: " + lookupValue);
                }
                resultsContainer.Click();
                ThinkTime(1000);
                //Click Add
                driver.ClickWhenAvailable(By.XPath(AppElements.Xpath[AppReference.Dialogs.SimpleLookupAddButton]), TimeSpan.FromSeconds(5),
                    "Unable to click the OK button in the simple lookup dialog");
        ...
        internal BrowserCommandResult<bool> ClickCommand(string name, string subname = null, string subSecondName = null, int thinkTime = Constants.DefaultThinkTime)
        {
            return Execute(GetOptions($"Click Command"), driver =>
            {
                ...
                var ribbon = driver.WaitUntilClickable(By.XPath(AppElements.Xpath[AppReference.CommandBar.Container]),
                    TimeSpan.FromSeconds(5));

                if (ribbon == null)
                {
                    ribbon = driver.WaitUntilClickable(By.XPath(AppElements.Xpath[AppReference.CommandBar.ContainerGrid]),
                        TimeSpan.FromSeconds(5),
        ...
        public string GetSubgridHeaderTitle(string subGridName)
        {
            return this.Execute(GetOptions("Get SubGrid Header"), driver =>
            {
                // Find the SubGrid
                driver.TryFindElement(By.XPath(AppElements.Xpath[AppReference.Entity.SubGridContents].Replace("[NAME]", subGridName)), out var subGrid);

                if (subGrid == null)
                    throw new NotFoundException($"Unable to locate subgrid contents for {subGridName} subgrid.");

                // Check if grid header title present and return value
                if (subGrid.TryFindElement(By.XPath(AppElements.Xpath[AppReference.Entity.SubGridHeaderTitle].Replace("[NAME]", subGridName)), out var subGridCommandBar))
                {
                    return subGridCommandBar.Text;
                }
                else
                {
                    return null;
        ...
        internal BrowserCommandResult<bool> SetValue(MultiValuePickList pickList, FormContextType formContextType = FormContextType.Entity)
        {
            return Execute(GetOptions("Set Value"), driver =>
            {
                IWebElement fieldContainer = null;
                fieldContainer = ValidateFormContext(driver, formContextType, pickList.Name, fieldContainer);
                ...
                if (!found)
                    throw new NoSuchElementException($"Field with name {pickList.Name} does not exist.");

                foreach (string value in pickList.Values)
                {
                    SetInputValue(driver, input, value);
                    var optionValue = driver.WaitUntilClickable(By.XPath(AppElements.Xpath[AppReference.Entity.EntityPickListSelectValue].Replace("[NAME]", value)));
                    optionValue.Click();
                }
        ...
        internal BrowserCommandResult<bool> SetValue(LookupItem control, FormContextType formContextType, bool useJsScroll = false)
        {
            return Execute(GetOptions($"Set Lookup Value: {control.Name}"), driver =>
            {
                ...
                if (useJsScroll)
                {
                    driver.ExecuteJavaScript("arguments[0].scrollIntoView(arguments[1]);", fieldContainer, true);
                }
        ...
        /// <summary>
        /// Sets the value of a Lookup, Customer, Owner or ActivityParty Lookup which accepts only a single value (without confirmation).
        /// </summary>
        /// <param name="control">The lookup field name, value or index of the lookup.</param>
        /// <example>xrmApp.Entity.FillInValue(new Lookup { Name = "prrimarycontactid", Value = "Rene Valdes (sample)" });</example>
        internal BrowserCommandResult<bool> FillInValue(LookupItem control, FormContextType formContextType)
        {
            return Execute(GetOptions($"Fill In Lookup Value: {control.Name}"), driver =>
            {
                driver.WaitForTransaction();
        ...
        /// <summary>
        /// Clicks lookup search icon.
        /// </summary>
        /// <param name="control">The lookup field name, value or index of the lookup.</param>
        internal BrowserCommandResult<bool> ClickLookupSearchIcon(LookupItem control, FormContextType formContextType)
        {
            return Execute(GetOptions($"Click Lookup Search Icon"), driver =>
            {
                IWebElement fieldContainer = null;
                fieldContainer = ValidateFormContext(driver, formContextType, control.Name, fieldContainer);
                ClickLookupSearchIcon(driver, fieldContainer, control);
        ...
        private void SetLookUpByValue(ISearchContext container, LookupItem control)
        {
            ...
            {
                int retries = 0;
                while (flyoutDialog == null && retries < 3)
                {
                    var input = container.FindElement(By.TagName("input"));
                    input.SendKeys(Keys.Space);
                    input.SendKeys(Keys.Backspace);
                    flyoutDialog = container.WaitUntilClickable(byPathToFlyout);
                    retries++;
                }
                if (flyoutDialog == null)
                {
                    throw new NotFoundException($"Could not find flyout dialog for Lookup control '{controlName}', value '{control.Value}' may not exist");
                }
            }
        ...
        public BrowserCommandResult<bool> SetValue(BooleanItem option, FormContextType formContextType)
        {
            return this.Execute(GetOptions($"Set BooleanItem Value: {option.Name}"), driver =>
            {
                ...
                var hasEmptyCheckbox = fieldContainer.HasElement(By.XPath(AppElements.Xpath[AppReference.Entity.EntityBooleanFieldEmptyCheckboxContainer].Replace("[NAME]", option.Name)));
                ...
                hasFlipSwitch = hasEmptyCheckbox ? false : hasFlipSwitch; //flipSwitch and Toggle have the same container reference, so if it has a Toggle it is not a flipSwitch
                ...
                else if (hasEmptyCheckbox)
                {
                    var checkbox = fieldContainer.FindElement(By.TagName("input"));
                    checkbox.Click();
                }
                else if (hasList)
                {
        ...
        internal BrowserCommandResult<Field> GetField(string field, bool waitUntilAvailable)
        {
            return this.Execute(GetOptions($"Get Field"), driver =>
            {
                var fieldElement = waitUntilAvailable ? driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Entity.TextFieldContainer].Replace("[NAME]", field)))
                    : driver.FindElement(By.XPath(AppElements.Xpath[AppReference.Entity.TextFieldContainer].Replace("[NAME]", field)));
        ...
        internal BrowserCommandResult<string> GetValue(string field, bool isQuickForm = false)
        {
            return this.Execute(GetOptions($"Get Value"), driver =>
            {
                ...
                if(isQuickForm)
                {
                    var formContainer = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.QuickCreate.QuickCreateFormContext]));
                    fieldContainer = formContainer.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Entity.TextFieldContainer].Replace("[NAME]", field)));
                }
                else
                {
                    fieldContainer = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Entity.TextFieldContainer].Replace("[NAME]", field)));
                }
        ...
        public BrowserCommandResult<string> GetValue(LookupItem control, bool isQuickForm = false)
        {
            var controlName = control.Name;
            return Execute($"Get Lookup Value: {controlName}", driver =>
            {
                ...
                {
                    var formContainer = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.QuickCreate.QuickCreateFormContext]));
                    fieldContainer = formContainer.WaitUntilAvailable(By.XPath(xpathToContainer));
                }
                else
                {
                    fieldContainer = driver.WaitUntilAvailable(By.XPath(xpathToContainer));
                }
        ...
        internal BrowserCommandResult<string> GetValue(OptionSet control, bool isQuickForm = false)
        {
            var controlName = control.Name;
            return this.Execute($"Get OptionSet Value: {controlName}", driver =>
            {
                ...
                {
                    var formContainer = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.QuickCreate.QuickCreateFormContext]));
                    fieldContainer = formContainer.WaitUntilAvailable(By.XPath(xpathToFieldContainer));
                }
                else
                {
                    fieldContainer = driver.WaitUntilAvailable(By.XPath(xpathToFieldContainer));
                }
        ...
        internal BrowserCommandResult<bool> GetValue(BooleanItem option, bool isQuickForm = false)
        {
            return this.Execute($"Get BooleanItem Value: {option.Name}", driver =>
            {
                ...
                IWebElement fieldContainer;
                var xpathToContainer = AppElements.Xpath[AppReference.Entity.TextFieldContainer].Replace("[NAME]", option.Name);
                if (isQuickForm)
                {
                    var formContainer = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.QuickCreate.QuickCreateFormContext]));
                    fieldContainer = formContainer.WaitUntilAvailable(By.XPath(xpathToContainer));
        ...
        internal BrowserCommandResult<string> GetHeaderContainerValue(string label)
        {
            return Execute(GetOptions($"Get Header Container Value {label}"), driver =>
            {
                try
                {
                    var headerLabelElement = driver.FindElement(By.XPath(AppElements.Xpath[AppReference.Entity.Header.ContainerTextField].Replace("[NAME]", label)));
                    return headerLabelElement.Text;
                }
                catch (NoSuchElementException)
                {
                    return "";
        ...
        private static void TryRemoveLookupValue(IWebDriver driver, IWebElement fieldContainer, LookupItem control, bool removeAll = true, bool isHeader = false)
        {
            ...
            else if (!isHeader && !success)
            {
                ...
                var found1 = fieldContainer.TryFindElement(xpathToHoveExistingValue, out var existingList);
                if (found1)
                    existingList.Hover(driver);
                var xpathToHoveExistingValueButton = By.XPath(AppElements.Xpath[AppReference.Entity.LookupFieldHoverExistingValueButton].Replace("[NAME]", controlName));
                var found2 = fieldContainer.TryFindElement(xpathToHoveExistingValueButton, out var existingListButton);
                if (found2)
                    existingListButton.Click();
            }
        ...
        private static void ClickLookupSearchIcon(IWebDriver driver, IWebElement fieldContainer, LookupItem control)
        {
            var controlName = control.Name;
            fieldContainer.Hover(driver);
            var xpathToExpandButton = By.XPath(AppElements.Xpath[AppReference.Entity.LookupFieldExpandCollapseButton].Replace("[NAME]", controlName));
            ...
            fieldContainer.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.Entity.TextFieldLookupSearchButton].Replace("[NAME]", controlName))).Click();
        }
        ...
        internal BrowserCommandResult<bool> SearchLookupField(LookupItem control, string searchCriteria, bool useJsScroll = false)
        {
            return this.Execute(GetOptions("Search Lookup Record"), driver =>
            {
                ...
                SetValue(control, FormContextType.Entity, useJsScroll);
        ...
        internal List<string> GetLookupResults(LookupItem control)
        {
            var controlName = control.Name;
            var xpathToControl = By.XPath(AppElements.Xpath[AppReference.Entity.LookupResultsDropdown].Replace("[NAME]", controlName));

            return Execute(GetOptions($"Get Lookup Results"), driver =>
            {
                return GetListItems(driver.WaitUntilVisible(xpathToControl), control).Select(x => x.Text).ToList();
            });
        }
        ...
        internal BrowserCommandResult<Field> BPFGetStatusField()
        {
            return this.Execute(GetOptions($"Get Status Field"), driver =>
            {
                IWebElement fieldLabel = null;
                try
                {
                    fieldLabel = driver.FindElement(By.XPath(AppElements.Xpath[AppReference.BusinessProcessFlow.HeaderKpiLabel]));
                }
                catch (NoSuchElementException)
                {
                    // Swallow
                ...
                Field returnField = new Field(fieldLabel);
        ...
        internal BrowserCommandResult<string> GetActiveStage()
        {
            return this.Execute(GetOptions($"Get Active Stage"), driver =>
            {
                IWebElement stageLabel = null;
                try
                {
                    stageLabel = driver.FindElement(By.XPath(AppElements.Xpath[AppReference.BusinessProcessFlow.ActiveStageLabel]));
                }
                catch (NoSuchElementException)
                {
                    return "";
        ...
        internal BrowserCommandResult<bool> BPFSetValue(string field, string value)
        {
            return this.Execute(GetOptions($"Set BPF Value"), driver =>
            {
                ...
                else if (fieldContainer.FindElements(By.XPath("//span[@role='textbox']")).Count > 0)
                {
                    var textarea = fieldContainer.FindElement(By.XPath("//span[@role='textbox']/../.."));
                    textarea.Click();
                    var label = driver.WaitUntilAvailable(By.XPath($"//label[text()='{value}']"));
                    label.Click();
                }
                else
                {
        ...
        internal BrowserCommandResult<bool> NextStage(string stageName, Field businessProcessFlowField = null, int thinkTime = Constants.DefaultThinkTime, bool finish = false)
        {
            ThinkTime(thinkTime);
            ...
            return this.Execute(GetOptions($"Next Stage"), driver =>
            {
                ...
                foreach (var control in flyoutFooterControls)
                {
                    ...
                    else
                    {
                        ...
                        var finishButton = control.FindElement(By.XPath(AppElements.Xpath[AppReference.BusinessProcessFlow.FinishButton]));
                        finishButton.Click();
                    }
                }
        ...
        public List<Dictionary<string, string>> GetGlobalSearchRecords(string entity)
        {
            var resultsList = new List<Dictionary<string, string>>();
            return this.Execute(GetOptions($"Get Global Search Records"), driver =>
            {
                var searchTypeElement = driver.WaitUntilAvailable(By.XPath(AppElements.Xpath[AppReference.GlobalSearch.Type]), "The Global Search type is not available.");
                var searchType = searchTypeElement.GetAttribute("value");
                ...
                if (searchType == "1") //Categorized Search
                {
                    ...
                    {
                        var attributes = record.FindElements(By.XPath(".//label"));
                        var dict = new Dictionary<string, string>();
                        foreach(var attr in attributes)
                        {
                            dict.Add(attr.GetAttribute("data-id"), attr.Text);
                        }
